...
// Term IDs from generated grammar
private const val DESCENDANT_OP = 110
private const val UNIT = 1
private const val OPEN_ARG_LIST = 2
...
private val argList = ExternalTokenizer({ input, _ ->
    if (input.next == 40) { // '('
        val prev = input.peek(-1)
        if (isAlpha(prev) || isDigit(prev) || prev == 95 || prev == 45) {
            input.acceptToken(OPEN_ARG_LIST, 1)
        }
    }
})
...
private val specIdentifier = mapOf(
    "lang" to 40, "nth-child" to 40, "nth-last-child" to 40,
    "nth-of-type" to 40, "nth-last-of-type" to 40, "dir" to 40,
    "host-context" to 40, "and" to 244, "or" to 244, "not" to 74,
    "only" to 74, "url" to 86, "url-prefix" to 86, "domain" to 86,
    "regexp" to 86, "when" to 117, "selector" to 142, "from" to 172,
    "to" to 174
)
...
val lessParser: LRParser = LRParser.deserialize(
    ParserSpec(
        ...
        maxTerm = 133,
        nodeProps = listOf(
            listOf("isolate", -3, 3, 4, 30, ""),
            listOf("openedBy", 17, "(", 59, "{"),
            listOf("closedBy", 26, ")", 60, "}")
        ),
        propSources = listOf(lessHighlighting),
        skippedNodes = listOf(0, 3, 4),
        repeatNodeCount = 10,
        ...
        tokenizers = listOf(descendant, unitToken, argList, 0, 1, 2, 3, 4),
        topRules = mapOf("StyleSheet" to listOf(0, 5)),
        specialized = listOf(
            SpecializerSpec(
                term = 116,
