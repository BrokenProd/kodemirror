...
// Term constants
private const val INSERTED_SEMI = 177
private const val SPACE_TERM = 179
private const val IDENTIFIER = 184
private const val STRING_TERM = 12
private const val CLOSE_PAREN_TERM = 13
private const val NUMBER_TERM = 17
private const val RUNE_TERM = 20
private const val CLOSE_BRACE_TERM = 25
private const val CLOSE_BRACKET = 53
private const val INC_DEC_OP = 95
private const val RETURN_TERM = 142
private const val BREAK_TERM = 144
private const val CONTINUE_TERM = 145
private const val FALLTHROUGH_TERM = 148
...
private const val CLOSE_PAREN = 41
private const val CLOSE_BRACE = 125

private val semicolon = ExternalTokenizer(
    { input, stack ->
        var scan = 0
        var next = input.next
        while (true) {
            if (stack.context as? Boolean == true &&
                (
                    next < 0 ||
                        next == NEWLINE ||
                ...
                input.acceptToken(INSERTED_SEMI)
            }
...
private val trackedTokens = setOf(
    INC_DEC_OP, IDENTIFIER, RUNE_TERM, STRING_TERM, NUMBER_TERM,
    BREAK_TERM, CONTINUE_TERM, RETURN_TERM, FALLTHROUGH_TERM,
    CLOSE_PAREN_TERM, CLOSE_BRACKET, CLOSE_BRACE_TERM
)

@Suppress("UNCHECKED_CAST")
private val trackTokens = ContextTracker(
    start = false,
    shift = { context, term, _, _ ->
        if (term == SPACE_TERM) context else trackedTokens.contains(term)
    }
) as ContextTracker<Any?>

// Specializer map for Go keywords
